...
export default async function QRCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: student }, { data: org }] = await Promise.all([
    supabase.from("members")
      ...
      .eq("id", id).eq("organisation_id", orgUser.organisation_id).single(),
    supabase.from("organisations")
      .select("name, slug, logo_url, primary_color")
      .eq("id", orgUser.organisation_id).single(),
  ]);
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="flex items-center gap-3">
        <Link href={`/students/${id}`} className="btn-ghost p-2"><ArrowLeft size={16} /></Link>
        <div>
          <h2 className="page-title">QR Card Designer</h2>
          <p className="page-sub">{student.full_name}</p>
        </div>
      </div>
      <QRCardClient student={student} schoolName={org?.name ?? "School"} schoolSlug={org?.slug ?? "school"} studentId={id} />
    </div>
  );
}