...
// Student type inline — no external import needed
import {
...
export default function QRCardClient({
  ...
  student: { id: string; full_name: string; class_name: string | null; qr_code: string; employee_id: string | null };
  schoolName: string; schoolSlug: string; studentId: string
}) {
  ...
  async function renderCardToBlob(): Promise<Blob | null> {
    ...
    if (bodyEl) {
      ...
      if (cfg.showClass && student.class_name) {
        ctx.fillStyle = cfg.classColor
        ctx.font = `400 ${cfg.classFontSize}px ${cfg.fontFamily}`
        ctx.textAlign = 'center'
        ctx.fillText(student.class_name, w / 2, textY)
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: `Attendance QR card for ${student.full_name} (${student.class_name})`,
          files: [file],
        })
        setShareStatus('done')
  ...
  return (
    <div className="qr-designer">
      ...
      <div className="qr-header">
        <Link href={`/students/${studentId}`} className="back-btn">
          <ArrowLeft size={15} />
        </Link>
        <div>
          <div className="qr-page-title">QR Card Designer</div>
          <div className="qr-page-sub">{student.full_name} · {student.class_name}</div>
        </div>
