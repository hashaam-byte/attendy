...
import {
  ...
  CalendarDays, TrendingUp, ChevronRight, Phone,
        ...
        {days.map((d) => {
          ...
          return (
            <div
              ...
              style={{ width: 16, height: 16, borderRadius: 3, background: bg, border: `1px solid ${border}`, cursor: "default" }}
            />
          );
        })}
      </div>
...
export default function ParentDashboardPage() {
  ...
  const attendancePct = totalScanned > 0
    ? Math.round(((presentCount + lateCount) / totalScanned) * 100)
    : 0;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030a05", fontFamily: "'DM Sans', system-ui, sans-serif", color: "white" }}>
      <style>{`
        ...
        .pd-card { background: rgba(255,255,255,0.025); border: 1px solid rgba(255,255,255,0.07); border-radius: 18px; }
        .pd-log-row { display:flex; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(255,255,255,0.05); transition:background 0.15s; gap:12px; }
      ...
      {/* Top bar */}
      <header style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(3,10,5,0.9)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "0 1rem", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button
            ...
            style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,255,255,0.05)", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            <RefreshCw size={13} style={refreshing ? { animation: "pdSpin 1s linear infinite" } : {}} />
          </button>
          <button
            onClick={handleLogout}
            style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 9, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.15)", color: "#f87171", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
          >
      ...
      <main style={{ position: "relative", zIndex: 1, maxWidth: 640, margin: "0 auto", padding: "1.5rem 1rem 4rem" }}>
              ...
              return (
                <button
                  ...
                  style={{
                    padding: "7px 16px",
                    borderRadius: 100,
                    background: isSelected ? stColor : "rgba(255,255,255,0.05)",
                    border:     `1px solid ${isSelected ? stColor : "rgba(255,255,255,0.1)"}`,
                ...
                >
                  ...
                  {/* Show school name if parent has children at multiple schools */}
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 320, gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: "50%", border: `2px solid rgba(34,197,94,0.15)`, borderTopColor: primary, animation: "pdSpin 0.9s linear infinite" }} />
          ...
          <>
            ...
            <div className="pd-card" style={{ ...s(60), padding: "1.5rem", marginBottom: "1rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                {/* Avatar */}
                <div style={{ width: 64, height: 64, borderRadius: "50%", background: `${primary}22`, border: `2px solid ${primary}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: primary, flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {/* School badge — always visible, especially important for multi-school parents */}
                  {orgInfo && (
                  ...
                  {todayLog ? (() => {
                    ...
                    const Icon = cfg.icon;
                    return (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, background: cfg.bg, border: `1px solid ${cfg.border}` }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)" }}>
            ...
            <div className="pd-card" style={{ ...s(160), padding: "1.25rem", marginBottom: "1rem" }}>
              ...
              <div style={{ height: 8, borderRadius: 100, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(attendancePct, 100)}%`, borderRadius: 100, background: attendancePct >= 75 ? `linear-gradient(90deg, #16a34a, #4ade80)` : `linear-gradient(90deg, #dc2626, #f87171)`, transition: "width 0.8s cubic-bezier(0.22,1,0.36,1)" }} />
              </div>
            ...
            <div className="pd-card" style={{ ...s(240), overflow: "hidden" }}>
              <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{logs.length} records</span>
              </div>
                ...
                logs.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="pd-log-row">
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: cfg.bg, border: `1px solid ${cfg.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color, background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: 6, padding: "2px 8px", flexShrink: 0 }}>
                        {cfg.label}
                      </span>
                    </div>
                  );
            ...
            {/* Info footer */}
            <div style={{ ...s(300), marginTop: "1.5rem", padding: "1rem 1.25rem", borderRadius: 14, background: "rgba(34,197,94,0.04)", border: "1px solid rgba(34,197,94,0.12)" }}>
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", lineHeight: 1.6 }}>
                <span style={{ color: primary, fontWeight: 600 }}>How this works:</span> Your child's QR card is scanned at the school gate each morning. For queries, contact {orgInfo?.name ?? "your school"} directly.
              </p>
