...
// REDESIGNED: Cinematic scanner UI with dual-line animation meeting in center
...
export function ScannerClient({
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("members").select("id, full_name, class_name, parent_phone, organisation_id, is_active, qr_code").eq("organisation_id", orgId).eq("member_type", "student");
      if (data) setCachedMembers(data as CachedMember[]);
    })();
  }, [orgId]);

  useEffect(() => { getQueuedScans().then((q) => setQueuedCount(q.length)); }, []);

  async function syncQueue() {
    const queued = await getQueuedScans();
    for (const { key, value } of queued) {
      try {
        const { error } = await supabase.from("attendance_logs").insert({ organisation_id: value.organisation_id, member_id: value.member_id, scan_type: "entry", status: value.status, late_reason: value.late_reason, device_id: "scanner-offline", scanned_at: value.scanned_at });
  ...
  function playBeep(type: "success" | "late" | "error" | "warning") {
    ...
    try {
      ...
      else if (type === "late") { osc.frequency.setValueAtTime(660, ctx.currentTime); }
      else if (type === "warning") { osc.frequency.setValueAtTime(440, ctx.currentTime); osc.frequency.setValueAtTime(330, ctx.currentTime + 0.15); osc.frequency.setValueAtTime(440, ctx.currentTime + 0.3); }
  ...
  async function handleScan(qrCode: string) {
    ...
    try {
      ...
      if (!member && isOnline) {
        const { data } = await supabase.from("members").select("id, full_name, class_name, parent_phone, organisation_id, is_active, qr_code").eq("qr_code", qrCode).eq("organisation_id", orgId).maybeSingle();
      ...
      if (!member.is_active) {
        playBeep("warning");
        setFlashColor("rgba(251,191,36,0.12)");
        setTimeout(() => setFlashColor(null), 800);
      ...
      if (isOnline) {
        ...
        const { data: existing } = await supabase.from("attendance_logs").select("id, scanned_at").eq("member_id", member.id).eq("organisation_id", orgId).eq("scan_type", "entry").gte("scanned_at", `${todayStart}T00:00:00`).limit(1);
        if (existing && existing.length > 0) {
          playBeep("error");
          setFlashColor("rgba(96,165,250,0.1)");
          setTimeout(() => setFlashColor(null), 800);
          setScanResult({ type: "duplicate", name: member.full_name, className: member.class_name ?? undefined, time: new Date().toLocaleTimeString(), message: `Already scanned at ${formatTime(existing[0].scanned_at)}` });
          setProcessing(false); clearResult(); return;
        }
      }
      ...
      playBeep("error");
      setScanResult({ type: "error", name: "Error", time: new Date().toLocaleTimeString(), message: "Something went wrong." });
      clearResult();
    }
    setProcessing(false);
  }
  ...
  async function recordScan(member: CachedMember, status: "present" | "late", reason: string, scannedAt: string) {
    const scanData: QueuedScan = { organisation_id: orgId, member_id: member.id, scan_type: "entry", status, late_reason: reason || null, device_id: "scanner-web", scanned_at: scannedAt, queued_at: new Date().toISOString() };

    if (isOnline) {
      const { error } = await supabase.from("attendance_logs").insert({ organisation_id: scanData.organisation_id, member_id: scanData.member_id, scan_type: scanData.scan_type, status: scanData.status, late_reason: scanData.late_reason, device_id: scanData.device_id, scanned_at: scanData.scanned_at });
      if (error) { await queueScan(scanData); setQueuedCount((c) => c + 1); }
      else { fetch("/api/notify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "arrival", member_id: member.id, org_id: orgId, is_late: status === "late", late_reason: reason }) }).catch(() => {}); }
    } else {
    ...
    const flashC = status === "present" ? "rgba(34,197,94,0.15)" : "rgba(251,191,36,0.12)";
    setFlashColor(flashC);
    setTimeout(() => setFlashColor(null), 800);
    setScanCount((c) => c + 1);
    const resultTime = new Date().toLocaleTimeString("en-NG", { hour: "2-digit", minute: "2-digit" });
    setScanResult({ type: status === "present" ? "success" : "late", name: member.full_name, className: member.class_name ?? undefined, time: resultTime, lateReason: reason || undefined });
    setRecentScans((prev) => [{ name: member.full_name, status, time: resultTime }, ...prev.slice(0, 4)]);
    clearResult();
  }
  ...
  async function submitLateReason() {
    ...
    setProcessing(true);
    const member: CachedMember = cachedMembers.find((m) => m.id === pendingLate.memberId) ?? { id: pendingLate.memberId, full_name: pendingLate.name, class_name: null, parent_phone: null, organisation_id: orgId, is_active: true, qr_code: "" };
    await recordScan(member, "late", lateReason, new Date().toISOString());
    setShowLateModal(false); setPendingLate(null); setLateReason(""); setProcessing(false);
  }
  ...
  return (
    ...
    >
      ...
      {flashColor && (
        <div
          className="sc-overlay-fade fixed inset-0 pointer-events-none z-50"
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button
            ...
            style={{ width: 30, height: 30, borderRadius: 8, background: "rgba(255,255,255,0.05)", border: "none", cursor: "pointer", color: soundEnabled ? primaryColor : "rgba(255,255,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
          ...
          {!isPublicScanner && (
            <button
              ...
              style={{ width: 30, height: 30, borderRadius: 8, background: "rgba(255,255,255,0.05)", border: "none", cursor: "pointer", color: "rgba(248,113,113,0.7)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}
            >
              <LogOut size={14} />
            </button>
          )}
      ...
      {!isOnline && (
        <div style={{ background: "linear-gradient(90deg, #92400e, #78350f)", color: "#fef3c7", textAlign: "center", fontSize: 12, fontWeight: 600, padding: "8px 16px", letterSpacing: "0.02em" }}>
          📡 Offline mode — scans are queued and will sync on reconnect
        </div>
      )}
        ...
        <div style={{ width: "100%", position: "relative" }}>
          ...
          >
            <span style={{ position: "relative", width: 8, height: 8, display: "inline-block", flexShrink: 0 }}>
              <span style={{
                ...
                background: processing ? "#fbbf24" : primaryColor,
          ...
          >
            ...
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
              {/* Animated scan lines — meets at center */}
              {!processing && (
                <>
              ...
              {/* Vignette */}
              <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at center, transparent 55%, rgba(0,0,0,0.5) 100%)", pointerEvents: "none" }} />
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${currentResult.border}`,
                borderRadius: 18,
                padding: "16px 18px",
                display: "flex", alignItems: "center", gap: 14,
                boxShadow: `0 0 24px ${currentResult.glow}20`,
              }}
            >
              <div
                style={{
                  width: 48, height: 48,
                  borderRadius: "50%",
                  background: `${currentResult.glow}15`,
                  border: `1.5px solid ${currentResult.border}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  flexShrink: 0,
                  boxShadow: `0 0 16px ${currentResult.glow}30`,
                }}
              >
                <ResultIcon size={22} style={{ color: currentResult.iconColor }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: currentResult.labelColor, marginBottom: 2, letterSpacing: "0.04em" }}>
                  {currentResult.label}
                </p>
                <p style={{ fontSize: 16, fontWeight: 800, color: "rgba(255,255,255,0.92)", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {scanResult.name}
                </p>
                {scanResult.className && (
                  <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{scanResult.className}</p>
                )}
                {scanResult.message && (
                  <p style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", marginTop: 2, lineHeight: 1.5 }}>{scanResult.message}</p>
                )}
                {scanResult.lateReason && (
                  <p style={{ fontSize: 11, color: "#fbbf24", marginTop: 2 }}>Reason: {scanResult.lateReason}</p>
                )}
              </div>
        ...
        {recentScans.length > 0 && (
          <div style={{ width: "100%", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(255,255,255,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" }}>Recent Scans</span>
            </div>
            {recentScans.map((scan, i) => (
              <div
                key={i}
                className="sc-recent-item"
                style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "8px 14px",
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                  <div style={{
                    ...
                    background: scan.status === "present" ? "#4ade80" : "#fbbf24",
                    boxShadow: `0 0 6px ${scan.status === "present" ? "#4ade80" : "#fbbf24"}`,
                  }} />
                  <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {scan.name}
                  </span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                  <span style={{ fontSize: 10, color: scan.status === "present" ? "#4ade80" : "#fbbf24", fontWeight: 600 }}>
                ...
                {LATE_REASONS.map((r) => (
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${lateReason === r ? "rgba(251,191,36,0.5)" : "rgba(255,255,255,0.08)"}`,
                      background: lateReason === r ? "rgba(251,191,36,0.12)" : "rgba(255,255,255,0.03)",
                      color: lateReason === r ? "#fbbf24" : "rgba(255,255,255,0.5)",
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  cursor: processing ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif",
